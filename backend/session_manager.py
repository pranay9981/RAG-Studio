...
ARCH_INFO: Dict[str, Dict] = {
    ...
    "02 Graph RAG (Knowledge Graphs)": {
        ...
        "how": "Uses the LLM to extract (entity → relationship → entity) triples and builds a NetworkX knowledge graph. At query time it walks the graph for matching entities and combines it with dense vector results.",
    ...
    "05 Multimodal RAG (Vision + Text)": {
        ...
        "tagline": "Stores images in metadata; sends text + image to Llama 4 Scout vision for answers",
        "how": "When an image is uploaded, the LLM generates a text summary for embedding. The base64 image is stored in ChromaDB metadata. At query time retrieved chunks include both text and raw image.",
    ...
    "06 Multilingual RAG (BGE-M3)": {
        ...
        "how": "Uses a multilingual sentence-transformer so all languages share the same vector space. A cross-encoder re-ranks results before Llama 4 Scout answers in the same language as the query.",
    ...
    "07 RAG-Fusion (Query Expansion)": {
        ...
        "how": "Llama 4 Scout generates 4 different phrasings of your query. Each sub-query retrieves its own ranked list. All four are merged with Reciprocal Rank Fusion — documents appearing in multiple lists get boosted.",
    ...
    "08 HyDE RAG (Hypothetical Document)": {
        ...
        "how": "Llama 4 Scout generates a hypothetical ideal answer as if it were in your document. That hypothetical is embedded and used to retrieve real chunks closest to it — bridging the vocabulary gap between questions and documents.",
    ...
    "09 Structured RAG (CSV/Excel)": {
        ...
        "how": "When CSV or Excel files are ingested, the LLM generates pandas code from the natural language query and executes it directly on the DataFrame. Falls back to vector search for narrative questions. Both results are combined for the final answer.",
